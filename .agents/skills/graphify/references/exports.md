...
This starts a stdio MCP server that exposes tools: `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path`. Add it to an MCP-compatible agent orchestrator so other agents can query the graph live.

Clients cannot run shell substitution such as `$(...)` inside JSON configuration. Under `uv tool install`, the system `python3` may also be unable to import Graphify, so set `command` to the **absolute interpreter path** stored in `graphify-out/.graphify_python`:
```json
{
  "mcpServers": {
    "graphify": {
