...
> Core movement entry points resolve as of 2026-09-07 at `fcaf410d`, and the stale combat-notifier path was repaired. Revalidate numeric tuning and detailed behavior against live data assets before implementation.
...
Because **Roll** and **Sprint** share a single physical input binding (Space on keyboard / B on gamepad), [`PlayerInputReader.cs`](../../../Assets/Scripts/Entities/Character/Input/PlayerInputReader.cs) disambiguates user intent through hold duration:
...
- **1-Slot Action Buffer**: The state machine holds the latest actionable input in [`CharacterActionStateMachine`](../../../Assets/Scripts/Entities/Character/Runtime/CharacterActionStateMachine.cs).
...
In [`MovementComponent.cs`](../../../Assets/Scripts/Components/Movement/MovementComponent.cs):
...
Authoritative values defined in [`Assets/Settings/Player/MovementData.asset`](../../../Assets/Settings/Player/MovementData.asset):
...
- **Combat Sprint Stamina Drain**: Sprinting drains stamina ($10.0\text{ pts/s}$) only when `ICombatStateNotifier` in `Assets/Scripts/Orchestrators/Core/ICombatStateNotifier.cs` reports `CombatState.Combat`. Out of combat, sprinting incurs zero stamina cost.
...
- **Probing Method**: [`MovementComponent.TryProbeGround`](../../../Assets/Scripts/Components/Movement/MovementComponent.cs) uses `Physics.SphereCastNonAlloc` with a preallocated 8-element hit buffer.
- **Walkable Threshold**: Surfaces with slope angle $\le \text{controller.slopeLimit}$ are flagged as walkable.
- **Downward Snapping**: [`MaintainGroundContact()`](../../../Assets/Scripts/Components/Movement/MovementComponent.cs) snaps the controller downward up to `GroundSnapDistance = 0.35m` while already grounded, preventing false airborne detachment on stairs.
...
In [`Character.cs`](../../../Assets/Scripts/Entities/Character/Character.cs), movement and control locks are managed through a unified bitmask enum:
