...
context_keys:
  - layer-architecture
aliases:
  - LAYER_SERVICE_SYSTEM_ARCHITECTURE
tags:
...
> Checked against live source on 2026-09-07 at `fcaf410d`. The service, enums, data asset, interaction consumer, and navigation-bake consumer resolve. Unity must still verify the current serialized `LayerData`, `TagManager`, and collision-matrix values.
...
| Boundary | Source or owner | Responsibility and consumers |
|---|---|---|
| Unity project settings | `TagManager.asset`, `DynamicsManager.asset` | Define physical layer indices and the collision matrix. |
| Shared serialized registry | `LayerData.asset` through `LayerData` | Maps `LayerName` to one assignable layer and `LayerMaskName` to shared query masks. |
| Runtime API | `LayerService : ILayerService` | Validates the registry and serves `PreviewRenderService` and `InteractionController`. |
| Feature-owned physics | `MovementData`, `EnemyBehaviourProfile` | Own ground-probe and line-of-sight masks; these are intentionally not global service keys. |
| Editor access | `LayerDataEditorProvider` | Supplies the NavMesh bake tool, configuration validator, and EditMode tests. |
