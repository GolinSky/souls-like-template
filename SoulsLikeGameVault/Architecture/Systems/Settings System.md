...
The **SoulsLikeTemplate** project uses a central `SettingsService` as the edit-session, persistence, validation, and commit coordinator. Runtime application remains delegated to domain services and appliers, preserving domain boundaries without denying the coordinator that now exists.

### Current Status
> [!NOTE]
> **Validated 2026-09-07 at `fcaf410d`:** `SettingsService` exists and owns `Current`, `Draft`, edit/apply/cancel state, persistence, validation, and display confirmation. It delegates audio, camera, controls, and graphics application to domain services. Later sections in this note still describe an earlier future-state design and remain `needs-review`.

```mermaid
flowchart TD
    UI[SettingsUiController] --> SS[SettingsService]
    SS --> Data[GameSettingsData]
    SS --> Save[ISaveService]
    SS --> Audio[IAudioService]
    SS --> Camera[ICameraService]
    SS --> Input[IInputService]
    SS --> Graphics[IGraphicsSettingsApplier]
    Data --> AudioData[Audio settings]
    Data --> CameraData[Camera settings]
    Data --> ControlData[Controls settings]
    Data --> GraphicsData[Graphics settings]
```
