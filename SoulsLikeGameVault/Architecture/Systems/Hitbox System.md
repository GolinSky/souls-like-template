...
> Checked against live source on 2026-09-07 at `fcaf410d`. `MeleeHitboxController`, `ResolveMeleeHitCommand`, `CombatDefenseComponent`, combat relays, and the player/enemy state behaviours resolve. Revalidate when the combat result contract changes.
...
| Phase | Owner | Output or next step |
|---|---|---|
| Attack window authoring | Player/enemy attack state behaviours and combat relay/executor | Opens `MeleeAttackData`; parry windows update defense state. |
| Spatial contact | `MeleeHitboxController` | Deduplicates trigger contacts and creates `MeleeHitRequest`. |
| Authoritative resolution | `ResolveMeleeHitCommand` | Reads/mutates defense and health, then returns `MeleeHitResult`. |
| Presentation | Combat relay/executor, animator, and audio components | Plays reactions, parry feedback, clashes, and impact sound. |
| Critical attack | `CriticalAttackController`, `CriticalTargetCommand`, `CriticalDamageStateBehaviour` | Runs a synchronized path that intentionally bypasses the normal hitbox pipeline. |
...
Trigger contact
  -> deduplicate target
  -> validate target and combat state
  -> invulnerability
  -> parry
  -> block / guard break
  -> health and kill result
  -> stance break
  -> poise stagger
  -> directional hit
  -> publish resolved result
```

The ordered breakdown below is the authoritative detail. This compact pipeline replaces the previous 32-edge diagram, which was too dense for a normal Obsidian pane.
...
| Stage | Initiator | Required collaboration | Result |
|---|---|---|---|
| Start | Fresh light attack in neutral state | `CharacterActionStateMachine` asks `CriticalAttackController.TryStart` | Buffered attacks are excluded. |
| Eligibility | `CriticalAttackController` | `CriticalTargetCommand` checks riposte opportunity or backstab position/state | One valid target and critical kind are selected. |
| Preparation | `CriticalAttackController` | Preview damage, cancel normal hitboxes, align actors, and call `BeginCritical` | Attacker and victim enter the synchronized lock. |
| Playback | `AnimatorComponent` | Plays the paired critical clips | Victim root motion remains disabled. |
| Impact | `CriticalDamageStateBehaviour` at normalized time `0.22` | Calls `ApplyCachedDamage`, which delegates to the target command | Cached damage is applied exactly once. |
| Completion | Animation state exit | Controller calls `EndCritical` and `CompleteCritical` | Movement and input are restored. |
