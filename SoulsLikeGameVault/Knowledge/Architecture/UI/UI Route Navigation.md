...
context_keys:
  - ui-navigation
aliases:
  - UI_Route_Navigation_Architecture
tags:
  - status/needs-review
---
...
> Checked against live source on 2026-09-07 at `fcaf410d`. `UiRouteStack` and the controller model exist; the base-controller path was repaired in this note. Remaining route and lifecycle claims are advisory until fully validated.
...
1. **Stack-Based LIFO Navigation**: Navigation operates as a Last-In-First-Out (LIFO) stack managed by [`UiRouteStack`](../../../../Assets/Scripts/Ui/Navigation/UiRouteStack.cs). Pushing a new route hides the current screen; popping a route cleanly restores the previous screen.
2. **Decoupled Hub-and-Spoke Pattern**: Sub-screen controllers (e.g. [`EquipmentUiController`](../../../../Assets/Scripts/Ui/Equipment/EquipmentUiController.cs), [`InventoryUiController`](../../../../Assets/Scripts/Ui/Inventory/InventoryUiController.cs), [`TravelUiController`](../../../../Assets/Scripts/Ui/Travel/TravelUiController.cs)) have zero direct dependencies on their parent routers or peer routes. They communicate exclusively via route interfaces and lifecycle events (e.g., `CloseRequested`).
...
4. **Centralized Router Hosts**: Host controllers (e.g. [`PauseNavigationUiController`](../../../../Assets/Scripts/Ui/PauseNavigation/PauseNavigationUiController.cs) and [`GraceUiController`](../../../../Assets/Scripts/Ui/Grace/GraceUiController.cs)) own the `UiRouteStack`, process global cancel/back inputs (`UiBackAction`), and manage game state synchronization (`GameState.Paused`, `GameState.OnGraceSit`, `GameState.Idle`).
...
Defined in [`Assets/Scripts/Ui/Navigation/IUiRoute.cs`](../../../../Assets/Scripts/Ui/Navigation/IUiRoute.cs):
...
Defined in [`Assets/Scripts/Ui/Navigation/UiRouteStack.cs`](../../../../Assets/Scripts/Ui/Navigation/UiRouteStack.cs):
...
|---|---|---|---|---|
| **Pause Navigation** | [`PauseNavigationUiController`](../../../../Assets/Scripts/Ui/PauseNavigation/PauseNavigationUiController.cs) | [`IPauseNavigationRoute`](../../../../Assets/Scripts/Ui/PauseNavigation/IPauseNavigationRoute.cs) | [`IEquipmentRoute`](../../../../Assets/Scripts/Ui/Equipment/IEquipmentRoute.cs), [`IInventoryRoute`](../../../../Assets/Scripts/Ui/Inventory/IInventoryRoute.cs), [`ISystemRoute`](../../../../Assets/Scripts/Ui/System/ISystemRoute.cs) | Gameplay Pause key, Equipment hotkey, Inventory hotkey |
| **Grace Navigation** | [`GraceUiController`](../../../../Assets/Scripts/Ui/Grace/GraceUiController.cs) | [`IGraceRoute`](../../../../Assets/Scripts/Ui/Grace/IGraceRoute.cs) | [`ITravelRoute`](../../../../Assets/Scripts/Ui/Travel/ITravelRoute.cs) | Rest at Grace interaction (`GameState.OnGraceSit`) |
...
- Sub-controllers and host controllers inherit from `UiController` in `Assets/Scripts/Ui/Base/UiController.cs` and implement `IInitializable`, `ITickable`, `IDisposable`.
...
- [[Knowledge/Architecture/UI/Pause Navigation|Pause Navigation]] — Detailed guide for the Pause Navigation Hub, sub-routes, and hotkeys.
- [[Knowledge/Architecture/UI/Grace Navigation|Grace Navigation]] — Detailed guide for the Grace Navigation Hub, fading transitions, and fast travel.
- [[Knowledge/Guides/UI/UI Code Build Guide|UI Code Build Guide]] — Step-by-step guide for creating UI Views, Presenters, Controllers, Prefabs, and Addressables.
