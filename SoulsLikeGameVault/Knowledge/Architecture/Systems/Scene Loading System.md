...
`GameOrchestrator` forwards calls. Menu/travel callers still prepare pending spawn state before requesting a load; the service gate does not protect that earlier mutation. See [[Work/Issues/Rejected Scene Transition Can Overwrite Pending Spawn Intent]] for the remaining cross-caller ownership defect.
