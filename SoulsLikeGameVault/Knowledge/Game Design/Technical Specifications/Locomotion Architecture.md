...
description: Technical specification and system architecture for character
  locomotion, action buffering, root motion, ground probing, and state machine
  capability gating.
version: 2.0.0
...
updated: 2026-09-07
aliases:
  - Locomotion Architecture Technical Specification
tags:
  - status/needs-review
---
...
- **Input Sharing**: Sprint and Roll share the primary dodge keybinding in [`PlayerInputReader.cs`](../../../../Assets/Scripts/Entities/Character/Input/PlayerInputReader.cs).
...
- **Key-Release Event ($t_{\text{hold}} < 0.30\text{ s}$)**: Dispatches [`CharacterAction.Roll(moveInput, cameraYaw)`](../../../../Assets/Scripts/Entities/Character/Runtime/CharacterAction.cs).

### 1.2 Action State Machine & 1-Slot Buffer
- **Buffer Model**: [`CharacterActionStateMachine`](../../../../Assets/Scripts/Entities/Character/Runtime/CharacterActionStateMachine.cs) implements a deterministic 1-slot buffer (`_bufferedAction`).
...
- **Root Motion Relay**: [`AnimatorRootMotionRelay.cs`](../../../../Assets/Scripts/Components/Animator/AnimatorRootMotionRelay.cs) intercepts `OnAnimatorMove()`.
- **Tags**:
...
Capability locks are unified under a bitmask enum in [`Character.cs`](../../../../Assets/Scripts/Entities/Character/Character.cs):
...
- **SphereCast Probing**: [`MovementComponent.TryProbeGround`](../../../../Assets/Scripts/Components/Movement/MovementComponent.cs) uses `Physics.SphereCastNonAlloc` with a preallocated array (`GROUND_PROBE_HIT_CAPACITY = 8`) to detect ground geometry without allocations.
- **Cast Geometry**:
...
Values authored in [`Assets/Settings/Player/MovementData.asset`](../../../../Assets/Settings/Player/MovementData.asset):
...
|---|---|---|
| **Input Buffer** | 15–30 frame sliding queue ($250\text{--}500\text{ms}$) | 1-slot buffer with 1.0s retention in [`CharacterActionStateMachine`](../../../../Assets/Scripts/Entities/Character/Runtime/CharacterActionStateMachine.cs) |
| **Movement Locking** | Bitwise hex flags (`0x01, 0x02, 0x04, 0x08`) | [`MovementLockReason`](../../../../Assets/Scripts/Entities/Character/Character.cs) bitmask + Animator Tags (`"RootMotion"`, `"MovementBlocked"`) |
| **Stairs & Ground Alignment** | 2-Bone Inverse Kinematics (IK) & Pelvis adaptation | Pure kinematic downward snap ([`GroundSnapDistance = 0.35m`](../../../../Assets/Scripts/Components/Movement/MovementData.cs)) and `SphereCastNonAlloc` |
...
| **Weight Tiers & i-Frames** | Light/Med/Heavy/Overloaded i-frame tables | Not yet segmented by weight load; invulnerability managed by [`CombatDefenseComponent`](../../../../Assets/Scripts/Entities/Combat/CombatDefenseComponent.cs) |
