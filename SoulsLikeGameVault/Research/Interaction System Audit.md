---
title: Interaction System Audit
type: research
domains:
  - interaction
status: needs-review
...
context_keys:
  - interaction-audit
aliases:
  - INTERACTION_SYSTEM_ANALYSIS_REPORT
tags:
  - status/needs-review
  - research/audit
updated: 2026-09-14
...
1. **Entity Identity (`IEntity` / `ViewEntity`):** Every interactive actor or world object is assigned a unique 64-bit identifier (`Id`) and registered in the centralized [`EntityLocator`](../../Assets/Scripts/Entities/BaseEntity/EntityLocator.cs).
2. **Spatial Discovery (`IEntityLocator`):** Physics queries (colliders, triggers, raycasts) resolve the target entity through [`IEntityLocator.TryGetEntity(Collider, out IEntity)`](../../Assets/Scripts/Entities/BaseEntity/IEntityLocator.cs) via the [`IViewEntity`](../../Assets/Scripts/Entities/BaseEntity/ViewEntity.cs) hierarchy search.
3. **Behavioral Dispatch (`EntityCommand`):** Entities do not expose random public API methods or allow external callers to directly mutate their internal components. Instead, capabilities are encapsulated inside [`EntityCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/EntityCommand.cs) instances registered on the entity (`IEntity.TryGetComponent<TCommand>`).
...
- **Bypass of `IEntityLocator`:** [`InteractionController`](../../Assets/Scripts/Interactions/InteractionController.cs) runs `Physics.OverlapSphereNonAlloc`, but resolves candidates using Unity's `collider.GetComponentInParent<IInteractable>()`. It never consults [`IEntityLocator`](../../Assets/Scripts/Entities/BaseEntity/IEntityLocator.cs).
- **Interactables are not Entities:** [`GroundItem`](../../Assets/Scripts/Items/GroundItem.cs) and [`GraceView`](../../Assets/Scripts/Interactions/GraceView.cs) are standalone `MonoBehaviour` scripts without [`ViewEntity`](../../Assets/Scripts/Entities/BaseEntity/ViewEntity.cs), are not in [`EntityType`](../../Assets/Scripts/Entities/BaseEntity/EntityType.cs), and are never registered in [`IEntityLocator`](../../Assets/Scripts/Entities/BaseEntity/IEntityLocator.cs).
- **Interactables lack Entity Commands:** 
  - [`GroundItem`](../../Assets/Scripts/Items/GroundItem.cs) has no entity command; it reaches into the *actor's* components to invoke [`GroundItemCollectionCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/GroundItemCollectionCommand.cs), directly inverting responsibility.
  - [`LadderView`](../../Assets/Scripts/Entities/Ladder/LadderView.cs) is registered as an `Entity` in [`LadderSystem`](../../Assets/Scripts/Entities/Ladder/LadderSystem.cs), but has **zero** entity commands registered. Its interaction bypasses the ladder entity to manipulate the actor's [`LadderClimber`](../../Assets/Scripts/Entities/Ladder/LadderClimber.cs) directly.
  - [`InteractionCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/InteractionCommand.cs) on the player is merely a pass-through wrapper delegating directly to `interactable.InteractAsync(actor)`.

| Flow | Discovery | Capability owner | Execution result |
|---|---|---|---|
| Combat reference | Hitbox asks `IEntityLocator` for the collider's `IEntity` | Target owns `ResolveMeleeHitCommand` | Command resolves damage, poise, and stance. |
| Audited interaction path | Controller used `GetComponentInParent<IInteractable>()` directly | Player-side `InteractionCommand`; target remained a MonoBehaviour | Ground item and ladder code reached back into player components. |
| Required target path | Controller asks `IEntityLocator` for the interactable `IEntity` | Target owns `IInteractableCommand` | Player command invokes one target-owned prompt/check/interaction flow. |
...
[`InteractionController`](../../Assets/Scripts/Interactions/InteractionController.cs) handles candidate detection and interaction lifecycle for the player:
...
2. **Unbound to Entities:** The candidate model [`InteractionCandidate`](../../Assets/Scripts/Interactions/InteractionController.cs#L243-L258) stores an `IInteractable` reference, completely unaware of whether the target has an `IEntity` identity.
3. **Actor Identity Asymmetry:** While [`InteractionController`](../../Assets/Scripts/Interactions/InteractionController.cs#L60) verifies that the *actor* is an `IEntity` with [`InteractionCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/InteractionCommand.cs), it treats the *target* as a raw MonoBehaviour.

---

### 2.2 Ground Items: `GroundItem` & `GroundItemCollectionCommand`

#### Current Implementation ([`GroundItem.cs`](../../Assets/Scripts/Items/GroundItem.cs))
- Inherits from `MonoBehaviour, IInteractable`.
- Does **not** have [`ViewEntity`](../../Assets/Scripts/Entities/BaseEntity/ViewEntity.cs).
- Is **not** registered in [`EntityLocator`](../../Assets/Scripts/Entities/BaseEntity/EntityLocator.cs).
- Does not exist in [`EntityType`](../../Assets/Scripts/Entities/BaseEntity/EntityType.cs) (which currently only lists `Player = 0`, `Enemy = 1`, `Ladder = 2`).
- In `InteractAsync`:
  ```csharp
...
1. **Inversion of Control & Responsibility Leakage:** The target MonoBehaviour reaches inside the actor to find a specific command ([`GroundItemCollectionCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/GroundItemCollectionCommand.cs)).
...
#### Current Implementation ([`LadderView.cs`](../../Assets/Scripts/Entities/Ladder/LadderView.cs), [`LadderEndpoint.cs`](../../Assets/Scripts/Entities/Ladder/LadderEndpoint.cs))
- [`LadderView`](../../Assets/Scripts/Entities/Ladder/LadderView.cs) **is** registered as an `Entity` of [`EntityType.Ladder`](../../Assets/Scripts/Entities/BaseEntity/EntityType.cs) in [`LadderSystem`](../../Assets/Scripts/Entities/Ladder/LadderSystem.cs#L96-L100).
- The root GameObject possesses a [`ViewEntity`](../../Assets/Scripts/Entities/BaseEntity/ViewEntity.cs).
- However, the trigger colliders for interaction are placed on child GameObjects containing [`LadderEndpoint`](../../Assets/Scripts/Entities/Ladder/LadderEndpoint.cs) (Top and Bottom mounts).
- [`LadderEndpoint`](../../Assets/Scripts/Entities/Ladder/LadderEndpoint.cs) implements `IInteractable`, delegating calls directly to `LadderView`:
  ```csharp
  public UniTask InteractAsync(IEntity actor, CancellationToken token) =>
      _ladder.InteractAsync(actor, end, token);
  ```
- In [`LadderView.InteractAsync`](../../Assets/Scripts/Entities/Ladder/LadderView.cs#L107-L138):
  ```csharp
...
2. **Endpoint Bypasses Entity Architecture:** [`InteractionController`](../../Assets/Scripts/Interactions/InteractionController.cs) detects `LadderEndpoint` via `GetComponentInParent<IInteractable>()`. It never resolves the ladder through `IEntityLocator`.
...
#### Current Implementation ([`GraceView.cs`](../../Assets/Scripts/Interactions/GraceView.cs))
- Implements `IInteractable`.
- Possesses a `SphereCollider` on layer `Default`.
- Holds no `ViewEntity` and has no `Entity` in `EntityLocator`.
- All interactions delegate to [`IGracePresenter`](../../Assets/Scripts/Interactions/IGracePresenter.cs) ([`GraceSystem`](../../Assets/Scripts/Interactions/GraceSystem.cs)).
...
| Architectural Requirement | Combat Reference Standard ([`MeleeHitboxController`](../../Assets/Scripts/Entities/Combat/MeleeHitboxController.cs)) | Ground Items ([`GroundItem`](../../Assets/Scripts/Items/GroundItem.cs)) | Ladders ([`LadderEndpoint`](../../Assets/Scripts/Entities/Ladder/LadderEndpoint.cs)) | Grace ([`GraceView`](../../Assets/Scripts/Interactions/GraceView.cs)) | Required Interaction Standard |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
| **Has Target `EntityCommand`?** | Yes ([`ResolveMeleeHitCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/ResolveMeleeHitCommand.cs)) | **No** | **No** | **No** | **Yes (`target.TryGetComponent<IInteractableCommand>`)** |
| **Has Actor `EntityCommand`?** | Yes ([`TargetingCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/TargetingCommand.cs)) | Yes ([`GroundItemCollectionCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/GroundItemCollectionCommand.cs)) | No ([`LadderClimber`](../../Assets/Scripts/Entities/Ladder/LadderClimber.cs) is only component) | No | **Yes ([`InteractionCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/InteractionCommand.cs))** |
...
All interactable entities register an `EntityCommand` that implements a shared interface, e.g. [`IInteractableCommand`](../../Assets/Scripts/Interactions/IInteractable.cs):
...
- Ground Item: [`GroundItemInteractCommand : EntityCommand, IInteractableCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/GroundItemCollectionCommand.cs)
- Ladder: [`LadderInteractCommand : EntityCommand, IInteractableCommand`](../../Assets/Scripts/Entities/Ladder/LadderView.cs)
- Grace: [`GraceInteractCommand : EntityCommand, IInteractableCommand`](../../Assets/Scripts/Interactions/GraceView.cs)

---

### 4.2 Spatial Discovery Refactoring in `InteractionController`

Instead of querying `GetComponentInParent<IInteractable>()`, [`InteractionController`](../../Assets/Scripts/Interactions/InteractionController.cs) queries [`_entityLocator.TryGetEntity(collider, out IEntity targetEntity)`](../../Assets/Scripts/Entities/BaseEntity/IEntityLocator.cs):
...
sequenceDiagram
    ...
    Cmd->>Player: TryGetComponent(out GroundItemCollectionCommand or LadderClimber)
    Cmd-->>ActCmd: Complete
...
1. **Entity Representation:**
   - Add `GroundItem = 3` to [`EntityType`](../../Assets/Scripts/Entities/BaseEntity/EntityType.cs).
   - Attach [`ViewEntity`](../../Assets/Scripts/Entities/BaseEntity/ViewEntity.cs) to the [`GroundItem.prefab`](../../Assets/Prefabs/Models/Item/GroundItem.prefab) root.
...
3. **Lifecycle & Registration (`GroundItemSystem`):**
   - Create a `GroundItemSystem` (or integrate with existing scene registration) that generates a unique ID, constructs the `Entity`, registers `GroundItemInteractCommand`, and initializes the entity into [`IEntityLocator`](../../Assets/Scripts/Entities/BaseEntity/IEntityLocator.cs).
...
1. **Ladder Entity Command:**
   ...
   - Register it on `Ladder.Entity` in [`LadderSystem.Register(LadderView ladder)`](../../Assets/Scripts/Entities/Ladder/LadderSystem.cs#L97-L104).
...
- **Violation:** In [`Assets/Scripts/Entities/BaseEntity/Entity.cs`](../../Assets/Scripts/Entities/BaseEntity/Entity.cs#L8-L13), `public interface IEntity` is declared inside `Entity.cs` along with `public class Entity`.
...
1. Extract `IEntity` out of [`Entity.cs`](../../Assets/Scripts/Entities/BaseEntity/Entity.cs) into [`IEntity.cs`](../../Assets/Scripts/Entities/BaseEntity/IEntity.cs).
2. Create `IInteractableCommand.cs` under `Assets/Scripts/Entities/BaseEntity/EntityCommands/`.
3. Add `GroundItem = 3` and `Grace = 4` (or unified `Interactable = 3`) to [`EntityType.cs`](../../Assets/Scripts/Entities/BaseEntity/EntityType.cs).
...
4. Update [`InteractionCommand`](../../Assets/Scripts/Entities/BaseEntity/EntityCommands/InteractionCommand.cs) on Player to accept `IEntity targetEntity, IInteractableCommand command`.

### Phase 3: Ground Items Modernization
1. Update [`Assets/Prefabs/Models/Item/GroundItem.prefab`](../../Assets/Prefabs/Models/Item/GroundItem.prefab) to include [`ViewEntity`](../../Assets/Scripts/Entities/BaseEntity/ViewEntity.cs).
2. Implement `GroundItemInteractCommand : EntityCommand, IInteractableCommand`.
3. Implement `GroundItemSystem` (similar to [`LadderSystem`](../../Assets/Scripts/Entities/Ladder/LadderSystem.cs)) to register all ground item entities in loaded scenes into [`EntityLocator`](../../Assets/Scripts/Entities/BaseEntity/EntityLocator.cs).

### Phase 4: Ladder Modernization
1. Implement `LadderInteractCommand : EntityCommand, IInteractableCommand`.
2. In [`LadderSystem.Register()`](../../Assets/Scripts/Entities/Ladder/LadderSystem.cs#L97-L104), construct and register `LadderInteractCommand` onto the ladder entity.
3. Remove direct `IInteractable` dependency from [`LadderEndpoint`](../../Assets/Scripts/Entities/Ladder/LadderEndpoint.cs) or make it a metadata tag pointing to the parent entity.

### Phase 5: Grace Modernization
1. Add [`ViewEntity`](../../Assets/Scripts/Entities/BaseEntity/ViewEntity.cs) to [`GraceView.prefab`](../../Assets/Prefabs/Models/Grace/GraceView.prefab).
2. Create `GraceInteractCommand : EntityCommand, IInteractableCommand`.
3. In [`GraceSystem`](../../Assets/Scripts/Interactions/GraceSystem.cs), create and register an `Entity` for each `GraceView`.
...
2. Verify with [`LayerConfigurationValidator`](../../Assets/Scripts/Editor/LayerConfigurationValidator.cs) and Unity console check.
