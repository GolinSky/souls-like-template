---
title: Enemy Encounter System Audit
type: research
domains:
  - enemy
  - spawning
...
updated: 2026-09-07
aliases:
  - ENEMY_ENCOUNTER_SYSTEM_ANALYSIS_REPORT
tags:
  - research/audit
---
...
**Primary Target:** [`EnemyEncounterSystem.cs`](../../Assets/Scripts/Entities/Enemy/EnemyEncounterSystem.cs)  
**Related Components:** [`EnemySpawnPoint.cs`](../../Assets/Scripts/Entities/Enemy/EnemySpawnPoint.cs), [`EnemyFactory.cs`](../../Assets/Scripts/Entities/Enemy/EnemyFactory.cs), [`EnemyGroupCoordinator.cs`](../../Assets/Scripts/Entities/Enemy/EnemyGroupCoordinator.cs), [`EnemyActor.cs`](../../Assets/Scripts/Entities/Enemy/EnemyActor.cs), [`EnemyController.cs`](../../Assets/Scripts/Entities/Enemy/EnemyController.cs), [`EnemyAuthoringValidator.cs`](../../Assets/Scripts/Editor/EnemyAuthoringValidator.cs), [`CoreScope.cs`](../../Assets/Scripts/Services/VContainer/CoreScope.cs), [`CheatsUiController.cs`](../../Assets/Scripts/Ui/Cheats/CheatsUiController.cs)

---

## 1. Executive Summary

The **`EnemyEncounterSystem`** is currently the sole mechanism for placing, spawning, and respawning enemy entities in the SoulsLikeTemplate project. It acts as an authoring container on a scene GameObject or prefab, containing serialized references to an array of [`EnemySpawnPoint`](../../Assets/Scripts/Entities/Enemy/EnemySpawnPoint.cs) components, spawning them synchronously via [`EnemyFactory`](../../Assets/Scripts/Entities/Enemy/EnemyFactory.cs), and managing their lifecycle across world events (Grace rest, Player death).
...
| **DI Scope Scalability** | Registered as a single `[SerializeField]` instance in [`CoreScope.cs`](../../Assets/Scripts/Services/VContainer/CoreScope.cs). | **Blocker** | Fails with >1 encounter in a scene. Already bypassed via scene `autoInjectGameObjects` hack. |
...
| **AI Coordination Coupling** | Creates and owns an isolated [`EnemyGroupCoordinator`](../../Assets/Scripts/Entities/Enemy/EnemyGroupCoordinator.cs). | **High** | AI attack slots (pressure tokens) and ally alerts are locked to spawner prefab boundaries. |
...
flowchart TD
    subgraph Current_Monolithic_Architecture["Current Reality: Tightly-Coupled Spawner Monolith"]
        CS["CoreScope<br/>(holds one serialized encounter)"] -->|Injects| EES[EnemyEncounterSystem]
        EES -->|Owns and ticks| EGC["EnemyGroupCoordinator<br/>(pressure slots and alerts)"]
        EES -->|Serialized Array| ESP[EnemySpawnPoint 1..N]
        ESP -->|Scene transforms| PP["PatrolPoint GameObjects<br/>(transforms in scene tree)"]
        ...
        EF -->|Builds child VContainer scope| LS["LifetimeScope sub-container<br/>(per enemy)"]
...
- **Conflating Spawning with Combat Grouping:** Because the spawner is called an "Encounter", it assumes all enemies spawned together must fight together, locking the [`EnemyGroupCoordinator`](../../Assets/Scripts/Entities/Enemy/EnemyGroupCoordinator.cs) directly inside the spawner component.
...
The author of [`EnemySpawnPoint.cs`](../../Assets/Scripts/Entities/Enemy/EnemySpawnPoint.cs) explicitly left a code comment:
```csharp
//todo: fully rework
...
```
In [`EnemyEncounter.prefab`](../../Assets/Prefabs/Models/Enemy/EnemyEncounter.prefab), this manifests as:
...
3. **Dead Weight at Runtime:**
   - During [`EnemyFactory.CreateEnemy`](../../Assets/Scripts/Entities/Enemy/EnemyFactory.cs#L76), [`spawn.BuildPatrolPositions()`](../../Assets/Scripts/Entities/Enemy/EnemySpawnPoint.cs#L23-L37) extracts `Vector3` coordinates into an array:
     ```csharp
...
4. **No Reusability or Advanced Routing:**
   ...
   - The patrol logic in [`EnemyController.DecidePatrol`](../../Assets/Scripts/Entities/Enemy/EnemyController.cs#L659-L691) is hardcoded to a circular loop:
     ```csharp
...
Every [`EnemySpawnPoint`](../../Assets/Scripts/Entities/Enemy/EnemySpawnPoint.cs) directly serializes 4 separate asset dependencies:
```csharp
...
[`EnemySpawnPoint`](../../Assets/Scripts/Entities/Enemy/EnemySpawnPoint.cs#L14) serializes:
```csharp
[SerializeField] private int randomSeedOffset;
```
- **Purpose:** In [`EnemyRandomStreams.cs`](../../Assets/Scripts/Entities/Enemy/EnemyRandomStreams.cs), this offset is added to the profile's base random seed so that identical enemies don't make synchronized AI decisions (e.g. all attacking or dodging at the exact same millisecond).
- **The Problem:** Designers must manually type `0`, `1`, `2`, `3` into the Inspector for every enemy.
- **Excessive Validator Overhead:** In [`EnemyAuthoringValidator.cs`](../../Assets/Scripts/Editor/EnemyAuthoringValidator.cs#L788-L806), extensive code exists solely to traverse encounters and scenes to assert that no two spawn points share the same `(EnemyBehaviourProfile, randomSeedOffset)`.
...
Inspecting [`CoreScope.cs`](../../Assets/Scripts/Services/VContainer/CoreScope.cs#L23-L28):
```csharp
...
3. In [`DefaultLocation.unity`](../../Assets/Scenes/DefaultLocation/DefaultLocation.unity), there are already **two** encounter prefabs:
...
5. In [`CheatsUiController.cs`](../../Assets/Scripts/Ui/Cheats/CheatsUiController.cs#L43):
   ```csharp
...
In [`EnemyEncounterSystem.SpawnEnemies()`](../../Assets/Scripts/Entities/Enemy/EnemyEncounterSystem.cs#L125-L131):
```csharp
...
In [`EnemyEncounterSystem.SpawnEnemies()`](../../Assets/Scripts/Entities/Enemy/EnemyEncounterSystem.cs#L122-L124):
```csharp
...
[`EnemyGroupCoordinator`](../../Assets/Scripts/Entities/Enemy/EnemyGroupCoordinator.cs) manages:
...
- **Ally Alert Broadcasting:** When an enemy detects the player, calling [`BroadcastAllyAlert`](../../Assets/Scripts/Entities/Enemy/EnemyGroupCoordinator.cs#L51-L75) notifies allies in the group.
...
In [`EnemyEncounterSystem.DespawnEnemies()`](../../Assets/Scripts/Entities/Enemy/EnemyEncounterSystem.cs#L133-L153):
```csharp
...
flowchart TD
    subgraph Management_Layer["1. Management & Service Layer (Singleton / CoreScope)"]
        ESM["EnemySpawnService / PopulationManager<br/>pure C# service or VContainer singleton"]
        SS["ISaveService / WorldStateStore<br/>tracks permanently dead enemies"]
        GS["IGameStateNotifier<br/>Grace / death observer"]
        GS -->|Notifies| ESM
        SS -->|Provides Dead State| ESM
    end

    subgraph World_Authoring_Layer["2. Authoring & Scene Placement Layer"]
        ESG1["EnemySpawnGroup: Camp A<br/>self-registers with EnemySpawnService"]
        ESG2["EnemySpawnGroup: Camp B<br/>self-registers with EnemySpawnService"]
        ESG1 -->|Holds| ESP_New[EnemySpawnPoint Data]
        ESP_New -->|References| EAD["EnemyArchetypeDefinition<br/>ScriptableObject: prefab and data"]
        ESP_New -->|References| PR["PatrolRoute definition<br/>local handles or spline"]
    end
    ...
    subgraph Dynamic_Combat_Layer["4. Decoupled Combat Arbitration"]
        CA["Spatial CombatCoordinator<br/>dynamic pressure slots and ally alerts"]
...
2. **Eliminate Manual PRNG Seed Offset:**
   ...
   - Automatically compute random seeds at runtime using a deterministic hash:
     ...
     ```
   - Deprecate the manual seed validation in [`EnemyAuthoringValidator.cs`](../../Assets/Scripts/Editor/EnemyAuthoringValidator.cs).
...
3. **Death State Persistence:**
   - Integrate with `ISaveService` (similar to [`CharacterSpawnService`](../../Assets/Scripts/Services/Spawn/CharacterSpawnService.cs)).
