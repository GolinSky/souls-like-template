...
2. Read [[Agent Context Registry]]. Resolve an exact key when supplied; otherwise match the task signals column literally.
...
- Completed work produces a small record in `History/Implementation Records/` and updates the relevant current architecture note.
- Superseded material moves to `Archive/`; it is never silently deleted or treated as current.
