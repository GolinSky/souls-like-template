...
namespace Jorjouto.AnimComposerSystem
{
    ...
    public class AnimCoordinatorComponent : MonoBehaviour
    {
        ...
        /// </summary>
        public event Action<RuntimeAnimComposer> OnAnimStart;

        /// <summary>
        /// Event that is raised when an animation composer finishes or is stopped.
        /// </summary>
        public event Action<RuntimeAnimComposer> OnAnimEnd;

        /// <summary>
        /// Event that is raised when an animation composer is stopped.
        /// </summary>
        public event Action<RuntimeAnimComposer> OnAnimStop;

        /// <summary>
        /// Event that is raised when an animation composer finishes naturally.
        /// </summary>
        public event Action<RuntimeAnimComposer> OnAnimFinish;

        /// <summary>
        /// Event that is raised when foot ik is blocked.
        /// </summary>    
        [Tooltip("Event that is raised when foot ik is blocked.")]
        public event Action IKFootBlockedEvent;

        /// <summary>
        /// Event that is raised when foot ik is unblocked.
        /// </summary>    
        public event Action IkFootUnblockedEvent;

        /// <summary>
        /// Event that is raised when root motion triggers movement.
        /// </summary>    
        public event Action<Vector3> DisplacementRootMotionTriggeredEvent;

        /// <summary>
        /// Event that is raised when root motion triggers rotation.
        /// </summary>    
        public event Action<Quaternion> RotationRootMotionTriggeredEvent;
        ...
        /// <summary>
        /// Provides a target that the owner of this component will use for reference when motion warping.
        /// </summary>
        [Tooltip("Provides a target that the owner of this component will use for reference when motion warping.")]
        ...
        #if HAS_RIGGING
        /// <summary>
        /// The cached reference to the AcsRigBuilder component (if found).
        /// </summary>
        private AcsRigBuilder rigBuilder;
        #endif
        ...
        {
            #if HAS_RIGGING
            rigBuilder = GetComponent<AcsRigBuilder>();
            #endif
        ...
        private void SetupAnimations()
        {
            ...
            if(rigBuilder != null)
            {
                rigBuilder.Build(playableGraph);
            }
            #endif
        }
        ...
        /// </param>
        public RuntimeAnimComposer PlayAnimComposer(ScriptableObject_AnimComposer newAnimComposer,
        ...
        /// blending them out over the specified time.
        /// </summary>
        public void InterruptAllAnimComposers()
        {
            for (int i = 0; i < LayersAndActiveAnimComposers.Count; i++)
            {
        ...
        public void InterruptActiveAnimComposersInLayer(float blendOutTime, int layer)
        {
            ...
            for (int i = activeAnimComposers.Count - 1; i >= 0; i--)
            {
                ...
                if (bWasPlaying)
                {
                    ...
                    OnAnimStop?.Invoke(activeAnimComposers[i]);
                }
            }
        }
        ...
        public void InterruptActiveAnimComposersInLayer(int layer)
        {
            ...
            if(activeAnimComposers.Count == 0)
            {
                return;
            }
        ...
        public void PlayAnimatorState( string stateName, 
                                       ...
                                       bool bShouldInterruptActiveComposers = true)
        {
            if (locomotionPlayable.IsValid())
            {
                if(bShouldInterruptActiveComposers)
                {
                    InterruptAllAnimComposers();
                }
        ...
        public void PlayAnimatorStateWithBlend( string stateName, 
                                                int layer = 0, 
                                                float fixedTime = 0f, 
                                                float normalizedTime = 0f, 
                                                bool bShouldInterruptActiveComposers = true)
        {
            if (locomotionPlayable.IsValid())
            {
                if(bShouldInterruptActiveComposers)
                {
                    InterruptAllAnimComposers(fixedTime);
                }

                locomotionPlayable.CrossFadeInFixedTime(stateName, fixedTime, layer, normalizedTime);
            }
        }

        //State name hash overrides (more performant)
        public void PlayAnimatorState( int stateNameHash, 
                                       int layer = 0, 
                                       float normalizedTime = 0f, 
                                       bool bShouldInterruptActiveComposers = true)
        {
            if (locomotionPlayable.IsValid())
            {
                ...
                locomotionPlayable.Play(stateNameHash, layer, normalizedTime);
            }
        }

        public void PlayAnimatorStateWithBlend( int stateNameHash, 
                                                int layer = 0, 
                                                float fixedTime = 0f, 
                                                float normalizedTime = 0f, 
                                                bool bShouldInterruptActiveComposers = true)
        {
            if (locomotionPlayable.IsValid())
            {
                ...
                locomotionPlayable.CrossFadeInFixedTime(stateNameHash, fixedTime, layer, normalizedTime);
            }
        }
        ...
        private float HandleBlendOutLayer(RuntimeAnimComposer animComposer, int index, float blendOutTime, int layer)
        {
            if (animComposer.IsPlaying)
            {
                ...
                OnAnimFinish?.Invoke(animComposer);
            }
        ...
        {
            DisplacementRootMotionTriggeredEvent?.Invoke(deltaPosition);
        ...
        {
            RotationRootMotionTriggeredEvent?.Invoke(deltaRotation);
