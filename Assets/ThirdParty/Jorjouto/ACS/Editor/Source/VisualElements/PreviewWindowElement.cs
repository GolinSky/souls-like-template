...
namespace Jorjouto.AnimComposerSystem.ACSEditor
{
    ...
    public partial class PreviewWindowElement : VisualElement
    {
        ...
        #if HAS_HDRP
        private HDAdditionalCameraData HDRPCamData = null;
        #endif
        ...
        public void UpdateManualCamera()
        {
            ...
            if(pressedKeys.Count > 0 && bIsRightMouseButtonHeld)
            {
                HandleFPSMovement();
            }
        }
        ...
        public void UpdatePreviewRenderUtility(ref GameObject previewObject, bool bResetCamera)
        {
            ...
            #if HAS_HDRP
            HDRPCamData = cam.gameObject.AddComponent<HDAdditionalCameraData>();
            HDRPCamData.clearColorMode = HDAdditionalCameraData.ClearColorMode.Color;
            HDRPCamData.backgroundColorHDR = animComposer.PreviewBackgroundColor;
            #endif
            ...
            // Keep the scene bright overall, but still allow the character to cast shadows onto the floor.
            PreviewRenderUtility.ambientColor = new Color(0.35f, 0.35f, 0.35f, 1f);
            ...
            Color lightColor = new(0.8f, 0.85f, 1f);
            ...
            fillLight.transform.rotation = Quaternion.Euler(35f, 315f, 0f);
        ...
        private void CreateColorFieldBindings()
        {
            ...
            previewBackgroundColor.RegisterValueChangedCallback(evt =>
            {
                if(PreviewRenderUtility != null)
                {
                    ...
                    #if HAS_HDRP
                    HDRPCamData.backgroundColorHDR = evt.newValue;
                    #endif
                    ReinitializePreviewTexture();
                }
            });
        ...
        private void HandleFPSMovement()
        {
            ...
            {
                cameraTarget += moveDirection.normalized * fpsCameraMovementSpeed * Time.deltaTime;//0.016f;
                UpdateCameraTransform();
            }
        }
        ...
        public void LoadPreviewItems(GameObject previewObject)
        {
            ...
            cachedPreviewRenderers.AddRange(previewObject.GetComponentsInChildren<Renderer>(true).Where(r => r.GetType().Name != "VFXRenderer"));
        ...
        private void SwitchObjectMaterialsToUnlit()
        {
            ...
            {
                #if HAS_HDRP
                unlitTextureShader = Shader.Find("HDRP/Unlit");
                #endif
            ...
            foreach (Renderer renderer in cachedPreviewRenderers)
            {
                ...
                for (int i = 0; i < shared.Length; i++)
                {
                    ...
                    {
                        // Standard color applied
                        unlitPreviewMat = new(unlitTextureShader);

                        if(mat.HasColor("_Color"))
                        {
                            unlitPreviewMat.color = mat.color;
                        }

                        // HDRP/URP texture switch
                        if (mat.HasProperty("_MainTex") && unlitPreviewMat.HasProperty("_MainTex"))
                        {
                            unlitPreviewMat.SetTexture("_MainTex", mat.GetTexture("_MainTex"));
                        }
                        // HDRP color switch
                        if(mat.HasProperty("_BaseColorMap") && unlitPreviewMat.HasProperty("_UnlitColorMap"))
                        {
                            unlitPreviewMat.SetTexture("_UnlitColorMap", mat.GetTexture("_BaseColorMap"));
                        }
                        // URP color switch
                        else if(mat.HasProperty("_BaseColor"))
                        {
                            unlitPreviewMat.SetColor("_Color", mat.GetColor("_BaseColor"));
                        }
