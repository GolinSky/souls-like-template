...
namespace Jorjouto.AnimComposerSystem.ACSEditor
{
    ...
    public class ActionBlockElement : VisualElement
    {
        ...
        /// <summary>
        /// The custom name given to the action by the user.
        /// </summary>
        ...
        private SerializedProperty actionBlockProperty;

        private readonly SerializedObject serializedAnimComposer;
        ...
        public ActionBlockElement(
            SerializedObject serializedAnimComposer,
            ...
            float frameRate)
        {
            this.serializedAnimComposer = serializedAnimComposer;
            ...
            if(data.Action != null)
            {
                ...
                if(ActionBlockData.Action.CustomName != null && ActionBlockData.Action.CustomName != "")
                {
                    actionDisplayName = ActionBlockData.Action.CustomName;
                }
                else
                {
                ...
                actionBlockProperty = serializedAnimComposer.FindProperty($"Tracks.Array.data[{TrackIndex}].ActionBlocks.Array.data[{ActionBlockIndex}].Action");
                this.TrackPropertyValue(actionBlockProperty.FindPropertyRelative("CustomName"), OnCustomNameChanged);

                label = new(actionDisplayName + " (" + ActionBlockData.StartFrame + " - " + ActionBlockData.EndFrame + ")");
        ...
        public void SelectActionBlock()
        {
            ...
            if (actionBlockProperty != null)
            {
                actionBlockProperty.isExpanded = true;

                PropertyField actionBlockPropertyField = new(actionBlockProperty)
                {
                ...
                panelStyle.display = DisplayStyle.Flex;
                actionBlockPropertyField.AddToClassList("action-blockData");

                actionBlockPropertyField.BindProperty(actionBlockProperty);
        ...
        /// <summary>
        /// Updates the ActionBlock label when the custom name field changes value
        /// </summary>
        /// <param name="customName">The new custom name assigned to the ActionBlock.</param>
        private void OnCustomNameChanged(SerializedProperty customName)
        {
            if(ActionBlockData.Action.CustomName != null && ActionBlockData.Action.CustomName != "")
            {
                actionDisplayName = ActionBlockData.Action.CustomName;
        ...
        private void OnMouseDownInsideBlock(MouseDownEvent evt)
        {
            ...
            else if (evt.button == 1)
            {
                ...
                menu.AddItem(new GUIContent("Edit Script (Z)"), false, EditScript);
        ...
        /// <summary>
        /// Handles the "Edit Script" context menu option to open the corresponding script in the editor.
        /// </summary>
        /// <param name="evt">The mouse event triggering the edit action.</param>
        public void EditScript()
        {
            Type type = ActionBlockData.Action.GetType();
        ...
        {
            label.text = actionDisplayName + " (" + ActionBlockData.StartFrame + " - " + ActionBlockData.EndFrame + ")";
        }
